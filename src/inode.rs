...
use crate::{FatDir, FatInode, FatInodeType, get_fat_data};
...
fn fat_mkdir(dir:Arc<Inode>, dentry: Arc<DirEntry>, _mode: FileMode) -> StrResult<()>{
    ...
    let inode = generate_fat_inode(sb_blk,FAT_INODE_DIR_OPS,FAT_DIR_FILE_OPS,InodeMode::S_DIR,parent_dir,current);
...
fn fat_create(dir:Arc<Inode>, dentry: Arc<DirEntry>, _mode: FileMode) -> StrResult<()>{
    ...
    // create a inode for the dentry
    let inode = generate_fat_inode(sb_blk,FAT_INODE_FILE_OPS,FAT_FILE_FILE_OPS,InodeMode::S_FILE,parent,current);
    // set the dentry's inode
    dentry.access_inner().d_inode = inode;
    Ok(())
}
...
// TODO! solve it
fn fat_rename(dir:Arc<Inode>, old_dentry: Arc<DirEntry>, new_dir: Arc<Inode>, new_dentry: Arc<DirEntry>) -> StrResult<()>{
    let _old_name = old_dentry.access_inner().d_name.clone();
    let _new_name = new_dentry.access_inner().d_name.clone();
    let _old_fat_data = get_fat_data(dir);
    let _new_fat_data = get_fat_data(new_dir);
    //todo
    Ok(())
}
...
fn generate_fat_inode(sb_blk:Arc<SuperBlock>, inode_ops:InodeOps, file_ops:FileOps, mode:InodeMode, parent:Arc<Mutex<FatDir>>, current:FatInodeType) ->Arc<Inode>{
    let inode = Inode::new(
        sb_blk,
        0,
        0,
...
fn __fat_create_dir_or_file(fat_data:&mut FatInode,is_dir:bool,name:&str)->Result<Arc<Mutex<FatDir>>,Error<()>>{
